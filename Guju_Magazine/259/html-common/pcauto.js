...
(function($){
  ...
  $(document).ready(function(){
    $(document.body).bind('touchstart', function(e){
      ...
      if(touch.according==false  && (touch.deltaX||touch.deltaY) )
		{
	...
	{	
	 if(touch.according == flip){
	   ...
	   { 
	   	   if(flip=="x"){
	   	    	if((touch.deltaY)<-20) {$(touch.target).trigger('swipeUp');touch.swipe=true};
		        if((touch.deltaY)>20 ) {$(touch.target).trigger('swipeDown');touch.swipe=true};
	        }else{
	        	if((touch.deltaX)>20) {$(touch.target).trigger('swipeRight'); touch.swipe=true;}
		        if((touch.deltaX)<-20) {;$(touch.target).trigger('swipeLeft');touch.swipe=true;}
	        }
	    }
	    e.preventDefault();
		e.stopPropagation();
