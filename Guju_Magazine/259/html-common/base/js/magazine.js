...
Magazine = (function(a_option){
		...
		*/
		...
		initScroller : function(){
			if(iScroll){
				var _aScroller = [];
				var _temp;
				$(".contentScroller").each(function(el,a_n){
		...
		initButton: function(){
			var that=this;
			var _temp;
			$(".button").each(function(a_el){
				$(a_el).tap(function(e){

					var e = window.event;
					if($(this).attr("data-href")){
					     that.callApp( $(this).attr("data-href"))   
					}
					e.preventDefault();
					e.stopPropagation();
					})
	...
	$(document.body).tap(function(e){
		...
		if(m._debug ){
			...
			//m.replaceImageIn();
		}else{m.callApp("http://PassOnClickEvent");}
	})
